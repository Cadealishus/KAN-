...
const HaloRingMod={
  ...
  teleportToRing(id,altKm){
    ...
    if(!global.flightState){ console.warn("[Halo] not in flight"); return false; }
    const THREE=this.state.THREE;
    const t=global.flightState.time;
    ...
    global.flightState.position.copy(c).add(new THREE.Vector3(r.radius-alt,0,0));
    global.flightState.velocity.copy(this.ringVelocity(r,t));
    global.flightState.crashed=false;
    global.flightState.landed=false;
  ...
  installCheatEntries(){
    ...
    if(global.hspLandBody&&!global.hspLandBody.__halo){
      ...
      const wrapped=function(id){
        if(typeof id==="string"&&id.indexOf("halo:")===0)
          return self.teleportToRing(id.slice(5),0.5);   /* just above the floor */
  ...
  removeCheatEntries(){
    const sel=document.getElementById("hspBodySel");
    if(sel) Array.from(sel.querySelectorAll('option[value^="halo:"]')).forEach(o=>o.remove());
